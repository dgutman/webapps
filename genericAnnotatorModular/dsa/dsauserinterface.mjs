...
export class DSAUserInterface extends OpenSeadragon.EventSource {
    constructor(viewer, options) {
        ...
        // Initialize dialog but don't show it - we need it for the API calls to work
        this.dialog.dialog({
            autoOpen: false,
            minWidth: 760,
        });
        ...
        // on pressing Go, inject content into sidebar instead of opening dialog
    ...
    _getAnnotatedImages() {
        this.API.get('annotation/images', { params: { limit: 10 } }, { noCache: true }).then(d => {
            ...
            // Look for annotated images container in sidebar first, fallback to dialog
            let container = $('#dsa-browser .dsa-annotated-images').length ?
                $('#dsa-browser .dsa-annotated-images') :
                this.dialog.find('.dsa-annotated-images');
    ...
    // private
    _getCollections() {
        console.log('_getCollections called, API:', !!this.API);
        if (!this.API) {
            console.log('No API available for collections');
            return;
        }

        this.API.get('collection', { params: { limit: 0 }, noCache: true }).then(d => {
            console.log('Collections API response:', d);

            // Look for collections container in sidebar first, fallback to dialog
            let contents = $('#dsa-browser .dsa-collections').length ?
                $('#dsa-browser .dsa-collections').empty() :
                this.dialog.find('.dsa-collections').empty();

            console.log('Collections container found:', contents.length);
            ...
            // Show error in the collections container
            let contents = $('#dsa-browser .dsa-collections').length ?
                $('#dsa-browser .dsa-collections').empty() :
                this.dialog.find('.dsa-collections').empty();
            contents.html('<div style="color: red;">Error loading collections: ' + error.message + '</div>');
        })
    }
    ...
    injectIntoSidebar() {
        // Inject the dialog content into the sidebar instead of showing as modal
        const sidebar = document.getElementById('dsa-browser');
        if (sidebar) {
            // Clear the sidebar
            sidebar.innerHTML = '';

            // Get the dialog content (without the modal wrapper)
            const dialogContent = this.dialog[0];
            if (dialogContent) {
                // Move the content to the sidebar
            ...
            console.log('Sidebar not found, falling back to modal');
