...
// Enhance DSA dialog with better draggable functionality
dsaUI.dialog.dialog({
    draggable: true,
    resizable: true,
    width: 600,
    minWidth: 500,
    maxWidth: Math.min(1000, window.innerWidth - 40),
    minHeight: 400,
...
// Update dialog constraints on window resize
$(window).on('resize', function () {
    if (dsaUI.dialog.dialog('isOpen')) {
        dsaUI.dialog.dialog('option', {
            maxWidth: Math.min(1000, window.innerWidth - 40),
            maxHeight: window.innerHeight - 40
        });
    }
...
function createViewer() {
    ...
    viewer.addHandler('page', ev => {
        ...
        // Setup keypress handlers after toolbar is initialized
