...
export class DSAUserInterface extends OpenSeadragon.EventSource {
    constructor(viewer, options) {
        ...
        // Render the DSA panel inside #dsa-sidebar
        const sidebar = $('#dsa-sidebar');
        sidebar.empty();
        sidebar.append(this.header);
        // Move the actual .dsa-annotated-images and .dsa-collections elements from this.dialog into the sidebar
        const annotatedImages = this.dialog.find('.dsa-annotated-images');
        const collections = this.dialog.find('.dsa-collections');
        const mainContent = $('<div class="dsa-main-content"></div>');
        mainContent.append(annotatedImages);
        mainContent.append(collections);
        sidebar.append(mainContent);
        sidebar.append(this.annotationEditorGUI);
        sidebar.css({
            position: 'relative',
            width: '100%',
            height: '100%',
        ...
        // Setup login/logout button in header (always visible)
        this._setupLoginHeaderButton();
        ...
        // on clicking Go, connect to DSA and show panel
        ...
        // Attach click handler to sidebar for .item elements
    ...
    _setupDSA(baseurl) {
        ...
        this.API.get('system/check?mode=basic').catch(e => {
            ...
            // No longer show login screen in sidebar; handled in modal/header
    ...
    // private
    _getAnnotatedImages() {
        // Always reference the sidebar's .dsa-annotated-images
        const annotatedImages = $('#dsa-sidebar .dsa-annotated-images');
        this.API.get('annotation/images', { params: { limit: 10 } }, { noCache: true }).then(d => {
            let element = $('<div>', { class: 'folder' });
            annotatedImages.empty().append(element);
    ...
    // private
    _getCollections() {
        // Always reference the sidebar's .dsa-collections
        const collections = $('#dsa-sidebar .dsa-collections');
        this.API.get('collection', { params: { limit: 0 }, noCache: true }).then(d => {
            let contents = collections.empty();
            let collectionsArr = d.map(collection => {
            ...
            contents.append(collectionsArr);
        })
    }
    ...
    _setupLoginHeaderButton() {
        const headerBtnContainer = $('#dsa-login-header-btn-container');
        headerBtnContainer.empty();
        let btn = $('<button id="dsa-login-header-btn">Login</button>');
        btn.on('click', () => {
            // Always create a new API instance if one does not exist
            if (!this.API) {
                this.API = new DigitalSlideArchiveAPI();
            }
            this._showLoginModal();
        });
        headerBtnContainer.append(btn);
        // If already logged in, update to show user info and logout
        if (this.API && this.API.userinfo) {
            btn.text(`Logout (${this.API.userinfo.user.login})`);
            btn.off('click').on('click', () => {
                this.API.LoginSystem.logout();
                this.API.userinfo = null;
                this._setupLoginHeaderButton();
            });
        } else if (this.API) {
            // Listen for login events to update button
            this.API.LoginSystem.getLoginScreen().on('logged-in', () => {
                this._setupLoginHeaderButton();
    ...
    _showLoginModal() {
        const modalContainer = $('#dsa-login-modal-container');
        modalContainer.empty();

        // Create a new API instance if we don't have one
        if (!this.API) {
            this.API = new DigitalSlideArchiveAPI();
        }

        const loginScreen = this.API.LoginSystem.getLoginScreen();
        // Only show the login form (not the sidebar intro)
        loginScreen.find('.dsa-intro').show();
        modalContainer.append(loginScreen.clone(true, true));
        ...
        modalContainer.find('#dsa-login-screen').css({
        ...
        modalContainer.find('#login-button').on('click', () => {
            setTimeout(() => {
                if (this.API && this.API.userinfo) {
                    this._hideLoginModal();
                }
